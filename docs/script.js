...
(function(){
  ...
  // XP/Level progression simulation helpers
  ...
  function reconcileProgressFromTotal(){
    // Normalize state across pages from xpTotal only
  ...
  // Initialize from total XP to ensure consistent state across pages
  reconcileProgressFromTotal();
  setLevel(levelIdx);
  if(xpTotalEl){ xpTotalEl.textContent = xpTotal.toLocaleString(); }

  // Replace addXp with dynamic rank progression
  function addXp(amount){
    ...
    if(didLevel){
      ...
      // Store current rank for notification system
...
function openProfileModal(){
  ...
  modal.querySelector('#profile-save')?.addEventListener('click', (e)=>{
    ...
    const md = document.getElementById('profile-modal');
    if(bd && md){ bd.classList.remove('open'); md.classList.remove('open'); }
  });
...
// Leaderboard names persistence and rendering
